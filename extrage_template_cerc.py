...
# Lista de extractii
extractii = [
    ('antrenare/1_20.jpg', '6B'),
    ('antrenare/2_20.jpg', '13P'),
    ('antrenare/3_20.jpg', '9G'),
    ('antrenare/4_20.jpg', '16O'),
    ('antrenare/5_20.jpg', '7P'),
    ('antrenare/5_20.jpg', '11L'),
    ('antrenare/4_19.jpg', '4D'),
]

output_folder = 'templates/cerc'
os.makedirs(output_folder, exist_ok=True)

for i, (filename, pos) in enumerate(extractii):
    ...
    print(f"Procesez {filename} pentru pozitia {pos}...")
    ...
    # APLICARE MASCA PE BOARD (Boardul modificat)
    ...
    cell = extrage_celula(masked_board, row, col)
    
    output_filename = os.path.join(output_folder, f"cerc_{i+1}.jpg")
    cv.imwrite(output_filename, cell)
    print(f"Salvat {output_filename}")

print("Gata!")

for idx, (img_path, pozitie) in enumerate(extractii, start=1):
    ...
    board = extrage_careu(img)
    if board is None:
        print(f"Nu am putut extrage careul din: {img_path}")
        continue
    
    row, col = pozitie_la_coord(pozitie)
    celula = extrage_celula(board, row, col, margin=0)
    
    # Salvam doar BGR - codul de detectie va converti la HSV automat
    cv.imwrite(f"{output_folder}/cerc_{idx}.jpg", celula)
    
    print(f"Extras: {img_path} pozitie {pozitie} -> cerc_{idx}.jpg")

print(f"\nTotal: {len(extractii)} template-uri extrase in {output_folder}/")
