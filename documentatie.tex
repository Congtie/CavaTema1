...
\textbf{Întreaga implementare se află într-un singur fișier Python}: \texttt{detectie\_piese\_qwirkle.py} (655 linii).
...
\begin{itemize}
\item \textbf{White (W)}: S≤50 și V≥60
\item \textbf{Red (R)}: H≥165 sau 155≤H<165
\item \textbf{Orange (O)}: H≤18
\item \textbf{Yellow (Y)}: 18<H≤35
\item \textbf{Green (G)}: 40≤H≤80
\item \textbf{Blue (B)}: 90<H≤130
\end{itemize}
...
Bucla principală din \texttt{detectie\_piese\_qwirkle.py} procesează imaginile din \texttt{antrenare/}:
...
\begin{lstlisting}[language=bash]
python detectie_piese_qwirkle.py
\end{lstlisting}

Rezultate în \texttt{detectate/}:
\begin{itemize}
...
\begin{lstlisting}[language=bash]
python evalueaza_detectie.py detectie_piese_qwirkle.py
\end{lstlisting}

Script-ul compară \texttt{detectate/} cu ground truth din \texttt{antrenare/}.

\section{Rezultate}

\begin{itemize}
\item \textbf{Training set}: 100\% acuratețe (8.00/8.00, 5 jocuri, 100 mutări)
\item \textbf{Fake test}: 99.4\% acuratețe (1.59/1.60, 1 joc, 20 mutări)
\end{itemize}

Abordarea HSV masking + multi-point sampling s-a dovedit robustă la variații de iluminare, perspectivă și umbre.
